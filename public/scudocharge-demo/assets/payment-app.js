(function () {
  ...
  function setBusy(isBusy, label) {
    ...
    document.querySelectorAll(".preset-button, .promo-button, .charger-card").forEach(function (buttonNode) {
  ...
  function renderChargerSelection() {
    ...
    var title = selected ? core.labelForCharger(selected) : "Choose a charger";
    var subtitle = selected ? core.describeCharger(selected) : "Choose the charger or connector your driver will use.";
    byId("selectedChargerName").textContent = title;
    byId("selectedChargerMeta").textContent = subtitle;
    byId("selectedSiteRef").textContent = selected ? (selected.siteId || state.config.site.siteId) : state.config.site.siteId;
    byId("selectedHardwareRef").textContent = selected ? ("Hardware ID " + selected.boxNum) : "Select a charger to reveal the hardware reference";
  }
  ...
  function renderAmount() {
    ...
    byId("summaryCharger").textContent = selected ? core.labelForCharger(selected) : "No charger selected";
  ...
  function renderPromotions() {
    ...
    packs.forEach(function (pack) {
      ...
      button.innerHTML =
        ...
        '<span class="promo-label">' + core.formatMoney(pack.credit - pack.pay, state.config) + " extra charging credit</span>";
      button.addEventListener("click", function () {
        if (state.busy) {
          return;
        }
  ...
  function renderChargerModal() {
    ...
    state.directory.forEach(function (charger) {
      var button = document.createElement("button");
      button.type = "button";
      button.className = "charger-card" + (String(state.selectedBoxNum) === String(charger.boxNum) ? " active" : "");
      button.innerHTML =
        '<span class="label">' + (charger.siteId || state.config.site.siteId) + "</span>" +
        "<strong>" + core.labelForCharger(charger) + "</strong>" +
        "<span>" + core.describeCharger(charger) + "<br>Hardware ID " + charger.boxNum + "</span>";
      button.addEventListener("click", function () {
        if (state.busy) {
          return;
        }
  ...
  function openModal() {
    renderChargerModal();
    byId("chargerModal").classList.add("open");
  }

  function closeModal() {
    byId("chargerModal").classList.remove("open");
  }
  ...
  async function submitPayment() {
    ...
    if (!selected) {
      setStatus("Select a charger before continuing to checkout.", "error");
      return;
    }
    ...
    setStatus("Checking charger availability and creating the payment session...", "");
  ...
  async function handleCheckoutReturn() {
    ...
    var apiBase = await core.resolveApiBase(state.config);
    setBusy(true, "Authorising charging session...");
  ...
  function bindEvents() {
    ...
    byId("selectChargerButton").addEventListener("click", openModal);
    ...
    byId("chargerModal").addEventListener("click", function (event) {
      if (event.target === byId("chargerModal")) {
        closeModal();
      }
    });
  }
