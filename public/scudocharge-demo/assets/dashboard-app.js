(function () {
  ...
  function buildQuery() {
    ...
    var charger = byId("filterCharger").value;
  ...
  function filterTransactionsForView(list) {
    var charger = byId("filterCharger") ? byId("filterCharger").value : "";
    ...
    return safeArray(list).filter(function (item) {
      ...
      if (charger && String(item.boxNum) !== String(charger)) {
        return false;
      }
  ...
  function fallbackPerformance(summary) {
    ...
    var byCharger = summary.byCharger || summary.byBay || {};
    return sortByRevenue(Object.keys(byCharger).map(function (boxNum) {
      var charger = core.findCharger(directory, boxNum);
      return {
        boxNum: boxNum,
        label: charger ? core.labelForCharger(charger) : "Charger " + boxNum,
  ...
  function renderChargerTotals(summary) {
    ...
    if (!performance.length) {
      list.innerHTML = '<div class="empty-state">No charger totals yet. Transactions will populate this view automatically.</div>';
      return;
    }
    performance.forEach(function (item) {
      var charger = core.findCharger(state.directory, item.boxNum);
      var tile = document.createElement("article");
      tile.className = "charger-tile";
      tile.innerHTML =
        '<div class="charger-head"><strong>' + (charger ? core.labelForCharger(charger) : item.label) + '</strong><span class="soft">' +
          core.formatMoney(item.revenue, state.config) + "</span></div>" +
        '<span class="muted">' + [charger ? charger.chargerId : item.chargerId, charger ? charger.location : "", charger ? charger.connector : ""].filter(Boolean).join(" • ") + "</span>" +
        '<div class="pill-row">' +
  ...
  function renderChargerPerformance(summary) {
    ...
    list.innerHTML = "";
    if (!performance.length) {
      list.innerHTML = '<div class="empty-state">Charger performance appears here once the network starts processing sessions.</div>';
      return;
    }
    var max = Math.max.apply(null, performance.map(function (item) { return Number(item.revenue || 0); }).concat([1]));
    performance.forEach(function (item) {
      var charger = core.findCharger(state.directory, item.boxNum);
      var label = charger ? core.labelForCharger(charger) : item.label;
      ...
      row.innerHTML =
        '<div class="performance-head"><div><strong>' + label + '</strong><div class="muted">' +
          [charger ? charger.chargerId : item.chargerId, charger ? charger.location : "", charger ? (charger.powerKw ? charger.powerKw + "kW" : "") : ""].filter(Boolean).join(" • ") +
  ...
  function renderAlerts(apiAlerts, transactions) {
    ...
    safeArray(apiAlerts).forEach(function (item) {
      var charger = core.findCharger(state.directory, item.boxNum);
      alerts.push({
        title: charger ? core.labelForCharger(charger) : "Charger " + item.boxNum,
  ...
  function renderTransactions(items) {
    ...
    items.forEach(function (item) {
      ...
      row.innerHTML =
        ...
        "<td><strong>" + (charger ? core.labelForCharger(charger) : ("Charger " + item.boxNum)) + "</strong><br><span class='soft'>" + (charger ? charger.siteId : state.config.site.siteId) + "</span></td>" +
  ...
  function renderFilters() {
    var select = byId("filterCharger");
    if (!select) {
      return;
    }
    var current = select.value;
    select.innerHTML = '<option value="">All chargers</option>';
    state.directory.forEach(function (charger) {
      ...
      if (chargerList.length) {
        state.directory = state.directory
          ...
          .concat(chargerList
            ...
            .map(function (boxNum) { return { boxNum: boxNum, chargerId: "CH-" + boxNum, label: "Charger " + boxNum, siteId: state.config.site.siteId }; }));
      }
