...
import {
  ...
  Lock, Unlock, Info, AlertTriangle
} from 'lucide-react'
  ...
  return (
    <div className="space-y-6 max-w-5xl">
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${account.is_active ? 'bg-teal/10' : 'bg-gray-100'}`}>
                    <User className={`w-5 h-5 ${account.is_active ? 'text-teal' : 'text-gray-400'}`} />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-gray-900 text-sm">{account.display_name}</p>
                    ...
                    {account.last_login_at && (
                      <p className="text-xs text-gray-400">
                        Last login: {new Date(account.last_login_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </p>
                    )}
                    {!account.last_login_at && (
                      <p className="text-xs text-amber-500">Never logged in</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-50 border border-slate-200">
                  ...
                  <span className={`font-mono text-sm font-bold tracking-widest ${showCode[account.id] ? 'text-gray-900' : 'text-slate-300 select-none'}`}>
                ...
                <div className="hidden xl:flex flex-wrap gap-1.5">
                  {Object.entries(account.permissions).filter(([, v]) => v).map(([k]) => (
                    <span key={k} className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors text-slate-600"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-lg hover:bg-red-50 transition-colors text-red-400 hover:text-red-600"
                  >
      ...
      <div className="flex items-center justify-between pt-6">
        <div>
          <h3 className="text-lg font-bold text-gray-900">Job offers</h3>
          <p className="text-xs text-gray-400">Optional: send upcoming shifts with hourly rate so staff can accept or decline.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            style={{ backgroundColor: '#0d9488' }}
          >
            <Plus className="w-4 h-4" /> New job offer
          </button>
        </div>
      </div>
      {jobOffersError && (
